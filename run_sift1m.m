% run siftmat
function [] = run_sift1m(k, num_bits, num_bits_subspace, pretrain_pq_opq_qpq, search_pq_qpq)
...
%matlabpool;
%% Load Dataset
basedir = '../dataset/sift1m/sift/';
X_train = fvecs_read([basedir 'sift_learn.fvecs']);
X_train = X_train';
X_base = fvecs_read([basedir 'sift_base.fvecs']);
X_query = fvecs_read([basedir 'sift_query.fvecs']);
ids = ivecs_read ([basedir 'sift_groundtruth.ivecs']);
ids_gnd = ids (1, :) + 1;
...
for n_trial = 1:trial
    ...
    if (search_pq_qpq == 'QPQ')
        ...
        tic; %t0 = cputime;
        [invertList, codeList, nonemptycell, centerDist] = qpq_assign_tce (pq, X_base_pq);
        %[invertList, nonemptycell, centerDist] = qpq_assign(pq, X_base_pq);
        tpqencode_vec(n_trial) = toc; %cputime - t0;
        
        tic; %t0 = cputime;
        %for s_iter = 1:1
          [ids_pqc] = qpq_search_tce(pq, nbase, invertList, codeList, nonemptycell, centerDist, X_query_pq, N);
        %end
        %[ids_pqc] = qpq_search(pq, nbase, invertList, codeList, nonemptycell, centerDist, X_query_pq, N);
        tpqsearch_vec(n_trial) = toc; %cputime - t0;
        fprintf('search_time:%.3f\n', tpqsearch_vec(1)/10);
        % compute search statistics
        nn_ranks_pqc = zeros (nquery, 1);
        hist_pqc = zeros (N+1, 1);
        for i = 1:nquery
...
%savename = ['result/recall_sift1m_pq_' '_k' num2str(k) '_' num2str(num_bits) 'bits_' num2str(M) 'M' '_' pretrain_pq_opq_qpq '_' search_pq_qpq '_' num2str(trial) 'trials' '.mat'];
