% run siftmat
function [] = run_sift1b(k, num_bits, num_bits_subspace, pretrain_pq_opq_qpq, search_pq_qpq)
...
%% Load Dataset
basedir = '../dataset/sift1b/';
X_train = b2fvecs_read([basedir 'bigann_learn.bvecs']);
X_train = X_train';
X_base = b2fvecs_read([basedir 'bigann_base.bvecs']);
X_query = b2fvecs_read([basedir 'bigann_query.bvecs']);
ids = ivecs_read ([basedir '/gnd/idx_1000M.ivecs']);
ids_gnd = ids (1, :) + 1;
...
savename = ['result/recall_sift1b' '_k' num2str(k) '_' num2str(num_bits) 'bits_' num2str(M) 'M' '_' pretrain_pq_opq_qpq '_' search_pq_qpq '.mat'];
