...
%% Load Dataset
basedir = '../dataset/gist1m/gist/';
X_train = fvecs_read([basedir 'gist_learn.fvecs']);
X_train = X_train';
X_base = fvecs_read([basedir 'gist_base.fvecs']);
X_query = fvecs_read([basedir 'gist_query.fvecs']);
ids = ivecs_read ([basedir 'gist_groundtruth.ivecs']);
ids_gnd = ids (1, :) + 1;
...
%savename = ['result/recall_gist1m' '_k' num2str(k) '_' num2str(num_bits) 'bits_' num2str(M) 'M' '_' pretrain_pq_opq_qpq '_' search_pq_qpq '.mat'];
%save(savename, 'mean_recall', 'recall', 'tpqlearn_vec', 'tpqencode_vec', 'tpqsearch_vec');
